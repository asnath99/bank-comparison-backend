...
const { BankProduct } = require('../models');


 // Lister tous les produits bancaires
...
const getActiveProducts = async (req, res) => {
  try {
    const products = await BankProduct.findAll({ where: { status: 'active' } });
...
const updateProductStatus = async (req, res) => {
  try {
    ...
    const product = await BankProduct.findByPk(id);
