...
// Récupérer uniquement les banques actives
const getActiveBanks = async (req, res) => {
  try {
    const banks = await Bank.findAll({ where: { is_active: true } }); // ⚠️ tu utilises is_active dans ton modèle
...
// Mettre à jour le statut d'une banque
const updateBankStatus = async (req, res) => {
  try {
    const { id } = req.params;
    const { is_active } = req.body; // ⚠️ ton champ est is_active, pas status
